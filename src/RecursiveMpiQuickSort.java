...
public class RecursiveMpiQuickSort {
	...
	static void parallelQuickSort(int arr[], int left, int right, int prRank, int maxRank, int rankIndex)
			throws MPIException {
		/* Calculate the rank of sharing process */
		int sharePr = prRank + (int) Math.pow(2, rankIndex);
		rankIndex++; // Increment the count index

		if (sharePr > maxRank) { // If no process to share sequentially
		...
		if (offset > arr.length - offset) {
			...
			parallelQuickSort(arr, left, offset - 1, prRank, maxRank, rankIndex);
			...
			parallelQuickSort(arr, offset, right, prRank, maxRank, rankIndex);
